...
class TelegramRepository:
    ...
    def delete_subscription(self, chat_id: str) -> None:
        connection = get_connection()
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM authorized_telegram_users WHERE chat_id = %s", (chat_id,))
        connection.commit()
        connection.close()
    ...
    def list_summary_subscribers(self) -> list[dict]:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT chat_id FROM authorized_telegram_users WHERE role IN ('admin', 'operator', 'viewer')"
            )
